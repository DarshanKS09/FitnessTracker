...
export default function FatsToday() {
  ...
  const fatsFoods = useMemo(
    () => logs.filter((item) => Number(item.fats || 0) > 0),
    [logs]
  );

  const totalFats = fatsFoods.reduce(
      ...
      <div className="max-w-4xl mx-auto">
        <div className="rounded-3xl border border-emerald-200 bg-white/95 shadow-lg p-5 sm:p-7 fx-card reveal" style={{ '--d': '40ms' }}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-emerald-900">Today&apos;s Fats Foods</h1>
              <p className="mt-1 text-sm text-emerald-700">
                Foods logged today with fats content
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="mt-4 rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3">
            <p className="text-sm text-emerald-700">
              Total fats today:{' '}
              <span className="font-bold text-emerald-900">{Math.round(totalFats)}g</span>
            </p>
          </div>
            ...
            <div className="mt-6 space-y-3">
              {fatsFoods.map((item) => (
                ...
                >
                  ...
                  <p className="text-lg font-bold text-emerald-800">{item.fats}g fats</p>
                </div>
              ))}
            </div>
          )}

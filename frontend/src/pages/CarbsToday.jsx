...
export default function CarbsToday() {
  ...
  const carbsFoods = useMemo(
    () => logs.filter((item) => Number(item.carbs || 0) > 0),
    [logs]
  );

  const totalCarbs = carbsFoods.reduce(
        ...
        <div className="rounded-3xl border border-emerald-200 bg-white/95 shadow-lg p-5 sm:p-7">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-emerald-700">
                Foods logged today with carbs content
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="mt-4 rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3">
            <p className="text-sm text-emerald-700">
              Total carbs today:{' '}
              <span className="font-bold text-emerald-900">{Math.round(totalCarbs)}g</span>
            </p>
          </div>
            ...
            <div className="mt-6 space-y-3">
              {carbsFoods.map((item) => (
                ...
                >
                  ...
                  <p className="text-lg font-bold text-emerald-800">{item.carbs}g carbs</p>
                </div>
              ))}
            </div>
          )}
